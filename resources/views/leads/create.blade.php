...
@section('content')
    <div class="w-full max-w-2xl mx-auto bg-white rounded-3xl shadow-2xl p-10 border border-gray-200">
        <h2 class="text-3xl font-extrabold text-center text-gray-900 mb-10">Cadastrar Lead</h2>
        ...
        <form method="POST" action="{{ route('leads.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4" required
                          class="w-full border-2 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl px-5 py-3 shadow-sm">{{ old('description') }}</textarea>
                @error('description')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
